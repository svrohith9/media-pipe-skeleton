...
import Calibration, { type CalibrationResult } from "../app/game/Calibration";
import Player from "../app/game/Player";
import Obstacle from "../app/game/Obstacle";
import ParallaxBg from "../app/game/ParallaxBg";
import Hud from "../app/game/Hud";
...
export default function GameStage() {
  ...
  const setThresholds = usePoseStore((state) => state.setThresholds);
  ...
  }, [setHighScore, setThresholds, thresholds]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (overlay) {
        ...
        if (ctx) {
          const videoWidth = video?.videoWidth ?? overlay.width;
          const videoHeight = video?.videoHeight ?? overlay.height;
          ...
          if (particleCanvas) {
            ...
            if (context) {
              ...
              for (const particle of particlePoolLocal) {
                ...
                context.arc(
                  particle.x,
                  particleCanvas.height - particle.y,
                  2,
                  0,
                  Math.PI * 2
                );
